...
1. Create Flask App
    1. Copy the following code into a new file called "app.py"
    ```
    ...
    ```
    This will create a local server where you can test the Flask App based on the route created in app.py. To stop the flaskenv use `deactivate`
2. Deploy Flask App to IBM Code Engine
    1. Follow these instructions to deploy your Flask App to IBM Code Engine: [Deploy a Python-Flask web application by using IBM Cloud Code Engine](https://developer.ibm.com/tutorials/deploy-a-python-flask-web-application-using-ibm-code-engine/)
    2. Once you have the Flask App deployed, take note of the endpoint that it is hosted at. 
3. Add Endpoint to API Spec
    1. You can now add the endpoint to the API Spec in Steps 5 and 6 of the [previous section](#download-and-import-the-oraclehcm-skillcustom-extensions-into-watsonx-orchestrate-ai-assistant).
