...
```mermaid
flowchart
    A(Call GET Worker queried to a single worker to retain workersUniqID from child URL)--> B(Call GET workRelationships to retain parameter PeriodOfServiceId property)-->C(Call GET workerAssignments to retain parameter assignmentsUniqID from child URL)-->D(Call PATCH on above path with required inputs)
```
...
```mermaid
flowchart
    A(Call GET Worker queried to a single worker to retain workersUniqID from child URL)--> B(Call GET workRelationships to retain parameter PeriodOfServiceId property)-->C(Call GET workerAssignments to retain parameter assignmentsUniqID from child URL)-->D(Call GET workerManagers to retain parameter managersUniqID from child URL)-->E(Call PATCH on above path with required inputs)
```
...
3. The optional query parameters must be set per the yaml comments or left blank
